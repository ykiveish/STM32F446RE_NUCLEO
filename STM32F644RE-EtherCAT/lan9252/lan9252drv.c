...
void SPIWriteProcRamFifo(spiCTX* ctx) {
   ...
   * Send address and length
	PRAM_WRITE_LEN	|	PRAM_WRITE_ADDR
  	31:16			|	15:0
  */
  SPIWriteRegisterDirect(ctx, ECAT_PRAM_WR_ADDR_LEN, (0x00001200 | (((uint32_t)TOT_BYTE_NUM_IN) << 16)));
  /*
   *
...
void SPIReadProcRamFifo(spiCTX* ctx) {
  ...
  SPIWriteRegisterDirect (ctx, ECAT_PRAM_RD_ADDR_LEN, (0x00001000 | (((uint32_t)TOT_BYTE_NUM_OUT) << 16)));
  // start command
